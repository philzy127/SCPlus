...
if ( ! class_exists( 'WPSC_DF_SCP_Recipient_Field' ) ) :

	final class WPSC_DF_SCP_Recipient_Field {
		 ...
		 * @var string
		 */
		public static $slug = 'scp_recipient_field';

        /**
         * Name for this custom field type
         *
         * @var string
         */
        public static $name = 'Recipient Field';
		...
		public static function set_rest_ticket_data( $data, $request, $custom_fields ) {
            ...
            foreach ( $custom_fields[ self::$slug ] as $cf ) {
                $recipients = $request->get_param( $cf->slug );
                if ( ! $recipients ) {
                    continue;
                }
                $recipients = array_unique(
                    array_filter(
                        array_map(
                            ...
                            explode( ',', sanitize_text_field( $recipients ) )
                        )
                    )
                );
                $data[ $cf->slug ] = implode( '|', $recipients );
            }
            return $data;
		}
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			$value      = $recipients && is_array( $recipients ) ? implode( ', ', $recipients ) : '';
			return apply_filters( 'wpsc_ticket_field_val_' . $cf->slug, $value, $cf, $ticket, $module );
		}
